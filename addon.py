...
import six #18
from six.moves import urllib_error, urllib_request, urllib_parse, http_cookiejar #18
...
def main_menu():
    ...
    for i in items:
        ...
        #li.setArt({'thumb': ch_img, 'poster': ch_img, 'banner': ch_img, 'icon': ch_img, 'fanart': ''})
...
def channelArrayGen():#JSON-live channel list
    ...
    resp=requests.post('https://hbb-prod.tvp.pl/apps/manager/api/hub/graphql',json=data)
    ch_data=json.loads(resp.text)['data']['getStationsForMainpage']['items']
    ar_chan=[]
    for c in ch_data:
        ch_code=c['code']
        ch_name=c['name']
...
def PlayChannel(chCode,chId):#play live channel
    if chCode!='':
        ...
        data=json.loads(strin)
        resp=requests.post('https://hbb-prod.tvp.pl/apps/manager/api/hub/graphql',json=data)
        if json.loads(resp.text)['data']['currentProgramAsLive'] is not None:
            streams=json.loads(resp.text)['data']['currentProgramAsLive']['formats']
        ...
        resp2=requests.post('https://hbb-prod.tvp.pl/apps/manager/api/hub/graphql',json=data2)
        streams2=json.loads(resp2.text)['data']['getLive']['data'][0]['formats']
        for s in streams2:
...
def replayChannelsArrayGen(): #JSON: replay channels
    ...
    data=json.loads(strin)
    resp=requests.post('https://hbb-prod.tvp.pl/apps/manager/api/hub/graphql',json=data)
    ch_data=json.loads(resp.text)['data']['getStations']['items']
    ar_chan=[]
    for ch in ch_data:
        chName=ch['name']
        chCode=ch['code']
...
def replayProgramsArrayGen(chCode,date):#JSON-programy wg daty
    ...
    data=json.loads(strin)
    resp=requests.post('https://hbb-prod.tvp.pl/apps/manager/api/hub/graphql',json=data)
    pr_data=json.loads(resp.text)['data']['getProgramsFromStation']['items']
    ar_prog=[]
...
def replayProgramsGen(chCode,date):#menu-replay program list
    ...
    for p in programs:
        ...
        #li.setArt({'thumb': p[2], 'poster': p[2], 'banner': p[2], 'icon': p[2], 'fanart': ''})
...
def getReplayProgramStreams(chCode,progID):
    ...
    data=json.loads(strin)
    resp=requests.post('https://hbb-prod.tvp.pl/apps/manager/api/hub/graphql',json=data)
    streams=json.loads(resp.text)['data']['programByRecordId']['formats']
    return streams
...
def generate_m3u(c):
    if file_name == '' or path_m3u == '':
        xbmcgui.Dialog().notification('TVPGO', 'Ustaw nazwe pliku oraz katalog docelowy.', xbmcgui.NOTIFICATION_ERROR)
        return
    xbmcgui.Dialog().notification('TVPGO', 'Generuje liste M3U.', xbmcgui.NOTIFICATION_INFO)
    data = '#EXTM3U\n'
    for item in c:
        channelCode = item[0]
        channelName = item[1]
    ...
    f.write(data)
    f.close()
    xbmcgui.Dialog().notification('TVPGO', 'Wygenerowano listę M3U.', xbmcgui.NOTIFICATION_INFO)
